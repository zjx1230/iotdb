...
//            String.format("CREATE TIMESERIES %s WITH DATATYPE=FLOAT,ENCODING=PLAIN", wholePath));
      ...
      statement.execute(
          String.format(
              "CREATE INDEX ON %s WITH INDEX=%s, SERIES_LENGTH=%d, FEATURE_DIM=%d, MAX_ENTRIES=%d, MIN_ENTRIES=%d, NO_PRUNE=NO_PRUNE",
...
//          System.out.println(insertSQL);
  ...
  public void checkRead() throws ClassNotFoundException {
    ...
    try (Connection connection =
      ...
      String gt =
          "Time,root.wind2.8.direction.(ND=3.87),root.wind2.9.direction.(ND=54.22),\n"
              + "0,120.0,135.0,\n"
              + "1,121.0,136.0,\n"
              + "2,122.0,137.0,\n"
              + "3,123.0,138.0,\n"
