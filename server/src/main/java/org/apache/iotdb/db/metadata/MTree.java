...
public class MTree implements Serializable {
   ...
   * Get measurement schema for a given path. Path must be a complete Path from root to leaf node.
   */
  void createIndex(PartialPath path, IndexInfo indexInfo) throws MetadataException {
    MeasurementMNode node = (MeasurementMNode) getNodeByPath(path);
    node.addIndexInfoMaps(indexInfo);
  }

  public void dropIndex(PartialPath fullPath, IndexType indexType) throws MetadataException {
    MeasurementMNode node = (MeasurementMNode) getNodeByPath(fullPath);
  ...
  public Map<String, Map<IndexType, IndexInfo>> getAllIndexInfosInStorageGroup(
      PartialPath storageGroup) throws MetadataException {
    Map<String, Map<IndexType, IndexInfo>> res = new HashMap<>();
    List<PartialPath> allTimeseriesInStorageGroup;

    allTimeseriesInStorageGroup = getAllTimeseriesPath(storageGroup);
    for (PartialPath fullPath : allTimeseriesInStorageGroup) {
      MeasurementMNode leafNode = (MeasurementMNode) getNodeByPath(fullPath);
      Map<IndexType, IndexInfo> infos = leafNode.getIndexInfoMaps();
      if (!infos.isEmpty()) {
        res.put(fullPath.getFullPath(), infos);
      }
    }
    return res;
  }

  public IndexInfo getIndexInfoByPath(PartialPath path, IndexType indexType) throws MetadataException {
    MeasurementMNode leafNode = (MeasurementMNode) getNodeByPath(path);
    Map<IndexType, IndexInfo> infos = leafNode.getIndexInfoMaps();
    if (!infos.isEmpty()) {
