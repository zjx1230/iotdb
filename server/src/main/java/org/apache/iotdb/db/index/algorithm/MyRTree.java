...
public class MyRTree<T> {
  ...
  public MyRTree(int nMaxPerNode, int nMinPerNode, int dim) {
    this(nMaxPerNode, nMinPerNode, dim, SeedsPicker.LINEAR);
  }

  public MyRTree(int nMaxPerNode, int nMinPerNode, int dim, SeedsPicker seedsPicker) {
  ...
  public static MyRTree<Integer> deserialize(ByteBuffer byteBuffer,
    ...
    MyRTree<Integer> rTree = new MyRTree<>(nMaxPerNode, nMinPerNode, dim, seedsPicker);
   ...
   */
  private static void deserialize(MyRTree rTree, RNode parent, ByteBuffer byteBuffer,
      BiConsumer<Integer, ByteBuffer> biConsumer) {
  ...
  private void toString(RNode node, int depth, StringBuilder sb) {
    ...
    sb.append("\n");
    if (!(node instanceof MyRTree.Item)) {
      for (RNode child : node.children) {
