...
public class ELBMatch extends IoTDBIndex {

  private static final Logger logger = LoggerFactory.getLogger(ELBMatch.class);
  private Distance distance;
  private ELBType elbType;

  private ELBMatchPreprocessor elbMatchPreprocessor;
  ...
  public ELBMatch(String path, IndexInfo indexInfo) {
  ...
  public void initPreprocessor(ByteBuffer previous, boolean inQueryMode) {
    ...
    this.elbMatchPreprocessor = new ELBMatchPreprocessor(tsDataType, windowRange, featureDim,
        elbType);
    this.indexPreprocessor = elbMatchPreprocessor;
  ...
  public List<Identifier> queryByIndex(ByteBuffer indexChunkData) throws IndexManagerException {
    ...
    try {
      windowBlockFeatures = ELBMatchPreprocessor.deserializeFeatures(indexChunkData);
    } catch (IOException e) {
  ...
  public int postProcessNext(List<IndexFuncResult> funcResult) throws IndexQueryException {
    ...
    if ( elbFeatureExtractor.exactDistanceCalc(aligned)){
      for (IndexFuncResult result : funcResult) {
        IndexFuncFactory.basicSimilarityCalc(result, indexPreprocessor, pattern);
      }
    }
