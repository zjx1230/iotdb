...
public abstract class IndexFeatureExtractor {
   ...
   * fields: {@code windowRange}, {@code windowRange}, {@code slideStep}.
   *
   * <p>COUNT_FIXED: TIME_FIXED: WHOLE_MATCH:
   *
   ...
   *
   * <p>Note that, after cleaning up all past store, the next {@linkplain #processNext()} will still
   ...
   * releaseSrcData}.
   *
   * <p>We do not call {@linkplain #clearProcessedSrcData} when triggering Sub-Flush, but use offset
   * to label how many point we have processed, because Sub-Flush may be triggered frequently when
   * the memory threshold is relatively small. If we use {@linkplain #clearProcessedSrcData}, we
   * need to move unprocessed data to position 0. The less data we flush each time, the more data we
   * need to move. Therefore, we only use appendSrcData when starting startFlushMemTable.
   */
